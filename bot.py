...
class MusicDatabase:
    ...
    def get_random_artists(self, count=4, exclude_id=None):
        """Получение нескольких случайных артистов"""
        with db_lock:
            conn = self.get_connection()
            cursor = conn.cursor()
            if exclude_id:
    ...
    # Методы для сравнения артистов
    def compare_artists(self, artist1_id, artist2_id):
        """Сравнение двух артистов"""
    ...
    def get_quiz_question(self, question_type):
        """Получение вопроса для викторины"""
        with db_lock:
            conn = self.get_connection()
            cursor = conn.cursor()
            ...
            if question_type == "genre":
                ...
                if artist:
                    ...
                    cursor.execute('''
                        ...
                        WHERE genre != ? ORDER BY RANDOM() LIMIT 3
                    ''', (artist['genre'],))
                ...
                cursor.execute('''
                    SELECT id, name, facts FROM artists 
                    ORDER BY RANDOM() LIMIT 1
                ''')
                artist = cursor.fetchone()
                if artist:
                ...
                cursor.execute('''
                    SELECT id, name, hits FROM artists 
                    WHERE hits IS NOT NULL AND hits != '' 
                    ORDER BY RANDOM() LIMIT 1
                ''')
                artist = cursor.fetchone()
                if artist:
                    ...
                    if hits:
                        ...
                        # Получаем другие песни
                        cursor.execute('''
                            SELECT hits FROM artists 
                            WHERE id != ? AND hits IS NOT NULL AND hits != ''
    ...
    def log_search(self, user_id, query, artist_found=False, artist_id=None):
        ...
        with db_lock:
            ...
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO search_history (user_id, query, artist_found, artist_id)
                VALUES (?, ?, ?, ?)
            ''', (user_id, query, artist_found, artist_id))
            
            if artist_found:
                cursor.execute('''
                    UPDATE users SET searches_count = searches_count + 1 
                    WHERE user_id = ?
                ''', (user_id,))
    ...
    def log_quiz_result(self, user_id, quiz_type, score, questions_played):
        """Логирование результатов викторины"""
        with db_lock:
            conn = self.get_connection()
            cursor = conn.cursor()
            ...
            cursor.execute('''
                SELECT best_quiz_score FROM users WHERE user_id = ?
            ''', (user_id,))
            result = cursor.fetchone()
            ...
            if score > current_best:
                cursor.execute('''
                    UPDATE users SET best_quiz_score = ? 
                    WHERE user_id = ?
                ''', (score, user_id))
            
            # Сохраняем результат викторина
            cursor.execute('''
                INSERT INTO quiz_results (user_id, quiz_type, score, questions_played)
                VALUES (?, ?, ?, ?)
    ...
    def get_user_stats(self, user_id):
        ...
        with db_lock:
            ...
            cursor.execute('''
                SELECT username, searches_count, quizzes_played, 
                       quiz_points, best_quiz_score, favorites_count
            ...
            # Получаем топ поисков
            cursor.execute('''
                SELECT query, COUNT(*) as count 
                FROM search_history 
                WHERE user_id = ? AND artist_found = 1
                GROUP BY query 
                ORDER BY count DESC 
                LIMIT 5
            ''', (user_id,))
            ...
            cursor.execute('''
                ...
                WHERE sh.user_id = ? AND sh.artist_found = 1
                GROUP BY a.genre
                ORDER BY count DESC
                LIMIT 1
            ''', (user_id,))
    ...
    def get_leaderboard(self, metric='quiz_points', limit=10):
        ...
        with db_lock:
            ...
            if metric == 'quiz_points':
                cursor.execute('''
                    SELECT username, quiz_points 
                    FROM users 
                    WHERE quiz_points > 0 AND username IS NOT NULL
                    ORDER BY quiz_points DESC 
                    LIMIT ?
                ''', (limit,))
            elif metric == 'searches':
                cursor.execute('''
                    SELECT username, searches_count 
                    FROM users 
                    WHERE searches_count > 0 AND username IS NOT NULL
                    ORDER BY searches_count DESC 
                    LIMIT ?
                ''', (limit,))
            elif metric == 'quizzes':
                cursor.execute('''
                    SELECT username, quizzes_played 
                    FROM users 
                    WHERE quizzes_played > 0 AND username IS NOT NULL
                    ORDER BY quizzes_played DESC 
                    LIMIT ?
                ''', (limit,))
            else:  # Общая активность
                cursor.execute('''
                    SELECT username, 
                           (quiz_points * 1.5 + searches_count * 0.5 + quizzes_played * 5) as activity_score
                    FROM users 
...
def format_artist_info(artist):
    """Форматирование информации об артисте"""
    if not artist:
    ...
    # Для групп показываем год основания, для сольных артистов - год начала карьеры
...
def callback_handler(call):
    ...
    try:
        if call.data == "main_menu":
        ...
        elif call.data == "show_genres_menu":
            ...
            artist_id = int(call.data.replace("artist_", ""))
            ...
            if call.data.startswith("quiz_type_"):
                quiz_type = call.data.replace("quiz_type_", "")
                if quiz_type == "random":
                    quiz_types = ["genre", "artist", "song"]
                    quiz_type = random.choice(quiz_types)
                
                start_quiz_session(chat_id, call.message.message_id, quiz_type, user_id)
            
            elif call.data == "quiz_stats":
                ...
                artist_id = int(call.data.replace("quiz_", ""))
                artist = db.get_artist_by_id(artist_id)
                if artist:
                    start_artist_quiz(chat_id, call.message.message_id, artist, user_id)
        
        elif call.data.startswith("compare_"):
            if call.data == "compare_random":
                artist1 = db.get_random_artist()
                artist2 = db.get_random_artist()
                
                # Убедимся, что артисты разные
                while artist2 and artist2['id'] == artist1['id']:
                    artist2 = db.get_random_artist()
                
                if artist1 and artist2:
                    show_comparison(chat_id, call.message.message_id, artist1, artist2)
                else:
                    bot.answer_callback_query(call.id, "Не удалось найти артистов для сравнения")
            
            elif call.data == "compare_select":
                msg = bot.send_message(
                    chat_id,
                    "Введите имена двух артистов через 'vs':\nПример: <code>Queen vs The Beatles</code>",
                    parse_mode='HTML'
                )
                bot.register_next_step_handler(msg, process_comparison_input)
            
            else:
                artist1_id = int(call.data.replace("compare_", ""))
                artist1 = db.get_artist_by_id(artist1_id)
                
                if artist1:
                    msg = bot.send_message(
                        chat_id,
                        f"Вы выбрали {artist1['name']}.\nТеперь введите имя второго артиста для сравнения:"
                    )
                    bot.register_next_step_handler(
                        msg, 
                        lambda m: process_comparison_with_first(m, artist1_id)
                    )
        
        elif call.data.startswith("leaderboard_"):
            metric = call.data.replace("leaderboard_", "")
            ...
            artist_id = int(call.data.replace("similar_", ""))
            artist = db.get_artist_by_id(artist_id)
            ...
            if artist and artist.get('similar_artists'):
                ...
                text += f"{similar}\n\n"
                text += "<i>Нажмите на имя артиста для подробной информации</i>"
                
                markup = types.InlineKeyboardMarkup()
                # Парсим имена похожих артистов
                similar_list = [s.strip() for s in similar.split(',')]
                for name in similar_list[:4]:
                    similar_artist = db.search_artist(name)
                    if similar_artist:
                        markup.add(types.InlineKeyboardButton(
                            f"🎵 {similar_artist['name']}", 
                            callback_data=f"artist_{similar_artist['id']}"
                        ))
...
def process_comparison_input(message):
    ...
    if not artist1 or not artist2:
        not_found = []
        if not artist1:
            not_found.append(artist1_name)
        if not artist2:
            not_found.append(artist2_name)
        
        bot.send_message(
            message.chat.id,
            f"Не найдены артисты: {', '.join(not_found)}"
        )
        return
...
def process_comparison_with_first(message, artist1_id):
    """Обработка сравнения с первым выбранным артистом"""
...
def show_comparison(chat_id, message_id, artist1, artist2):
    """Показать сравнение двух артистов"""
    ...
    # Определяем победителя по влиянию
...
def start_artist_quiz(chat_id, message_id, artist, user_id):
    """Начать викторину об артисте"""
    # Создаем вопросы об артисте
    questions = []
    
    # Вопрос о годе основания/начала карьеры
    if artist.get('formation_year') and not artist.get('is_solo_artist'):
        # Для групп
        questions.append({
            'type': 'year',
...
def generate_year_options(correct_year):
    ...
    for _ in range(3):
        ...
        while offset == 0:
            offset = random.randint(-15, 15)
        wrong_year = correct_year + offset
...
def generate_country_options(correct_country):
    ...
    # Добавляем неправильные варианты
    wrong_countries = [c for c in countries if c != correct_country]
    options.extend(random.sample(wrong_countries, min(3, len(wrong_countries))))
    
    random.shuffle(options)
    return options

def generate_genre_options(correct_genre):
    """Генерация вариантов жанров"""
    genres = db.get_all_genres()
    
    options = [correct_genre]
    
    # Добавляем неправильные варианты
    wrong_genres = [g for g in genres if g != correct_genre]
    options.extend(random.sample(wrong_genres, min(3, len(wrong_genres))))
...
def show_quiz_question(chat_id, message_id):
    ...
    text = f"🎮 <b>Вопрос {question_num}/{total_questions}</b>\n\n"
...
def process_quiz_answer(call):
    ...
    if is_correct:
        quiz['score'] += 10
        result_text = f"✅ <b>Правильно!</b> +10 очков\n"
    else:
        result_text = f"❌ <b>Неправильно.</b> Правильный ответ: {question['correct_answer']}\n"
    ...
    if quiz['current_question'] < len(quiz['questions']):
        ...
        # Показываем следующий вопрос через 1.5 секунды
        time.sleep(1.5)
...
def finish_quiz(chat_id, message_id):
    ...
    if total_points > 0:
        percentage = (quiz['score'] / total_points) * 100
    else:
        percentage = 0
    
    text = f"🎮 <b>Викторина завершена!</b>\n\n"
    text += f"🏆 <b>Итоговый счет:</b> {quiz['score']}/{total_points} очков\n"
    text += f"📊 <b>Результат:</b> {percentage:.0f}%\n\n"
    ...
    try:
        ...
        print(f"Ошибка при завершении викторины: {e}")
        bot.send_message(
            chat_id,
            text,
            parse_mode='HTML',
...
if __name__ == "__main__":
    print("="*60)
    print("🎶 БОТ 'ГОЛОС ВРЕМЕНИ' - ОБНОВЛЕННАЯ ВЕРСИЯ")
    print("="*60)
    print("✅ База данных инициализирована")
    print(f"✅ Загружено {len(db.get_all_genres())} жанров")
    print("✅ Исправлена ошибка с сольными артистами")
    print("✅ Бот готов к работе!")
    print("="*60)
    print("\n📊 Характеристики системы:")
    print(f"• 50+ артистов в базе данных")
    print("• 12+ музыкальных жанров")
    print("• Разделение на группы и сольных артистов")
    print("• Корректные вопросы в викторине")
    print("• Рабочие функции: поиск, викторина, сравнение")
    print("• Система статистики и таблица лидеров")
    print("="*60)
    print("\nОжидание сообщений...")
